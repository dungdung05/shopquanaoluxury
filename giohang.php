<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["add_to_cart"])) {
        ...
        $product_stock = $ProductModel->get_product_quantity($product_id); 
        ...
        if($product_quantity < 1 ) {
         ...
         if (isset($product_stock['quantity']) && $product_quantity > $product_stock['quantity']) {
        echo "<script>alert('Số lượng đặt vượt quá tồn kho. Chỉ còn {$product_stock['quantity']} sản phẩm');</script>";
        echo "<script>window.location.href='index.php?url=chitietsanpham&id_sp=".$product_id."&id_dm=16';</script>";
        exit();
    }
                                    ...
                                    <td class="cart__quantity" style="border-radius: 0;">
                                        <div class="input-group float-left" style="border-radius: 0; width: 124px;">
                                            <div class="input-next-cart d-flex" style="border-radius: 0; width: 124px;">
                                                ...
                                                <input type="number" readonly step="1" max="<?=$product_stock['quantity']?>"
